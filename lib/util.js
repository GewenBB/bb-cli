'use strict';
var fs = require('fs');
var chalk = require('chalk');
var clui = require('clui');
var moment = require('moment');

exports.backbase = function () {
    console.log(chalk.gray([' ____             _    _                     ', '| __ )  __ _  ___| | _| |__   __ _ ___  ___  ', '|  _ \\ / _` |/ __| |/ / \'_ \\ / _` / __|/ _ \\ ', '| |_) | (_| | (__|   <| |_) | (_| \\__ \\  __/ ', '|____/ \\__,_|\\___|_|\\_\\_.__/ \\__,_|___/\\___| '].join('\n') + '\n'));
};

exports.ok = function () {
    var args = Array.prototype.slice.call(arguments);
    if (args[1]) {
        args[1] = chalk.gray(args[1]);
    }
    args.unshift(chalk.gray('[' + moment().format('YYYY-MM-DD HH:mm:ss:SSS') + ']'));
    args.unshift(chalk.green('✓'));
    console.log.apply(this, args);
};

exports.warn = function () {
    var args = Array.prototype.slice.call(arguments);
    if (args[1]) {
        args[1] = chalk.gray(args[1]);
    }
    args.unshift(chalk.gray('[' + moment().format('YYYY-MM-DD HH:mm:ss:SSS') + ']'));
    args.unshift(chalk.yellow('!'));
    console.log.apply(this, args);
};

exports.err = function () {
    var args = Array.prototype.slice.call(arguments);
    if (args[1]) {
        args[1] = chalk.gray(args[1]);
    }
    args.unshift(chalk.gray('[' + moment().format('YYYY-MM-DD HH:mm:ss:SSS') + ']'));
    args.unshift(chalk.red('✖'));
    console.log.apply(this, args);
};

exports.getUserHome = function () {
    return process.env[(process.platform === 'win32') ? 'USERPROFILE' : 'HOME'];
};

exports.spin = new clui.Spinner('Please wait...');

exports.getAuthentication = function( bbrc ){
    var auth = {
        username: bbrc.username,
        password: bbrc.password,
        passphrase: bbrc.passphrase,
        securityOptions: bbrc.securityOptions
    };

    var pfx = bbrc.pfx;
    var cert = bbrc.certFile;
    var key = bbrc.keyFile;

    if(pfx){
        auth.pfx = readFile( pfx );

    } else if( cert && key ){

        auth.cert = readFile( cert );
        auth.key = readFile( key );
    }

    return auth;
};

function readFile( path ){
    return fs.readFileSync( path, 'utf8');
}
